...
#define COUNT_SIGNALS 63 // 43 + 20
#define COUNT_STORE 9
#include "Types.h"
#include "MgtClient.h"
#include "schedul.h"
#include "blockly.h"
...
static struct Signal* sRelay; // relay_1, relay_2, realy_3, relay_4
...
static struct Signal* sReverse; // reverse_1, reverse_2, reverse_3, reverse_4
...
static void handler(enum OpCode aOpCode, struct Signal* aSignal, struct SignalValue* aWriteValue) {
  switch (aOpCode) {
    case opRead:
      ...
      break;
    case opAttach:
    // логику отправки нужно тщательно прорабатывать, иначе в сеть и регистрацию пападают ложные данные.
/*      if ((aSignal >= sScheduleData) && (aSignal < sIPAddress)) {
        //readScheduleData(aSignal - sScheduleData, aOpCode);
      }
      else if ( ((aSignal >= sSensor) && (aSignal < sDI)) || ((aSignal >= sVariable) && (aSignal < sReverse)) ) { // если это 1-wire датчики и переменные, то отправляем только 
        if (aSignal->m_value.u.m_float == aSignal->m_value.u.m_float)
    ...
    case opWrite:
      ...
      if ((aSignal >= sReverse) && (aSignal < sScriptMode))
...
void setup() {
  ...
  mgt_createSignal(&client, "relay_3", tpBool, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_CHANGE | STORE_UNIT_MIN | 1, 0);
  mgt_createSignal(&client, "relay_4", tpBool, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_CHANGE | STORE_UNIT_MIN | 1, 0);
  ...
  mgt_createSignal(&client, "variable_3", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "variable_4", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "variable_5", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "variable_6", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "variable_7", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "variable_8", tpFloat, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  ...
  mgt_createSignal(&client, "reverse_3", tpBool, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  mgt_createSignal(&client, "reverse_4", tpBool, SEC_LEV_READ | SIG_ACCESS_READ | SIG_ACCESS_WRITE, STORE_MODE_OFF, 0);
  ...
  const char* ver = "PLC Sonoff 4CH v2.9 20/III/2021";
...
void loop() {
  ...
  if (!synchronization) {
    ...
    if (realTime) {
      shiftTime = realTime - (getUTCTime() - shiftTime);
      sch_init(realTime);
  ...
  if (!isAP) {
    ...
    bool relayDirty[4] = {false, false, false, false};
